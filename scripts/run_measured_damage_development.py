#!/usr/bin/env python3
"""Stage 3 seed-46 development evaluation at the 20% budget (CUDA production).

Requires a passing additivity gate. Evaluates the frozen
Measured-Damage-Robust allocations against every frozen comparator on the new
seed-46 development split for the additivity-authorized regime(s), applies the
five preregistered gates, and writes FINAL_CONFIRMATION_GO or
MEASURED_DAMAGE_NO_GO. Seed 44 is never touched here.
"""
...
from expert_analysis.measured_damage_reporting import (
    analyze_stage3_phase,
    create_stage3_figures,
    load_additivity_report,
    write_stage3_development_decision,
    write_stage3_phase_outputs,
...
def main() -> int:
    ...
    additivity = load_additivity_report(args.results_dir)
    authorized_regimes = list(
        additivity["additivity_decision"]["authorized_regimes"]
    )
    if not authorized_regimes:
        raise RuntimeError(
            "The additivity gate authorized no regime; the stage decision is "
            "MEASURED_DAMAGE_NO_GO and seed 46 must never be evaluated"
        )
    print(f"Additivity-authorized regimes: {authorized_regimes}")
    ...
    preregistration = read_json(args.results_dir / "stage3_preregistration.json")
    ...
    if additivity["registry_sha256"] != registry["registry_sha256"]:
        raise RuntimeError("The additivity report references a different registry")

    splits = {
        domain: load_stage3_development_split(args.results_dir / "splits", domain)
    ...
    if preregistration["development_split_input_hashes"] != split_hashes:
        raise RuntimeError("Seed-46 split hashes do not match the preregistration")
    print("Seed-46 development split verified against the preregistration.")

    references, competitors = stage3_phase_records(
        registry, allocations_dir, stage2b_allocations_dir, stage2c_allocations_dir,
        "development", authorized_regimes,
    )
    ...
    phase_dir = args.results_dir / stage3_phase_dir_name("development")
    ...
    paths = write_stage3_phase_outputs(analysis, phase_dir)
    for name, path in paths.items():
        print(f"Wrote {name}: {path}")
    decision_path = write_stage3_development_decision(
        analysis, additivity, args.results_dir, prereg_sha
    )
    decision = analysis["development_decision"]["decision"]
    print(f"Stage 3 development decision: {decision} ({decision_path})")
    ...
    if not args.skip_figures:
        figures = create_stage3_figures(
            args.results_dir, args.results_dir / "figures", analysis
        )
        print(f"Created {len(figures)} figure files.")
    write_stage3_summary(args.results_dir)
    if decision != "FINAL_CONFIRMATION_GO":
        print(
            "MEASURED_DAMAGE_NO_GO: stop here. Preserve all results; do not "
            "modify the objective, budgets, or damage matrix; do not inspect "
            "seed 44. This closes the measurement-based optimization "
            "hypothesis on this branch."
        )
    else:
        authorized = analysis["development_decision"]["authorized_regimes"]
        print(
            f"FINAL_CONFIRMATION_GO: regimes {authorized} are authorized for "
            "the seed-44 final confirmation. Run the independent audit before "
            "scripts/run_measured_damage_final.py."
        )
    return 0
