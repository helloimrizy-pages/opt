#!/usr/bin/env python3
"""Stage 3 seed-44 final confirmation (CUDA production; requires GO).
...
from expert_analysis.measured_damage_reporting import (
    ...
    write_stage3_final_decision,
    write_stage3_final_summary,
...
def main() -> int:
    ...
    decision_path = args.results_dir / "stage3_decision.json"
    if not decision_path.is_file():
        raise RuntimeError("Final evaluation requires stage3_decision.json")
    ...
    authorized_regimes = list(decision["development_decision"]["authorized_regimes"])
    ...
    if not audit_path.is_file():
        raise RuntimeError(
            ...
            "scripts/audit_measured_damage_preservation.py first"
        )
    ...
    phase_dir = args.results_dir / stage3_phase_dir_name("final")
    ...
    development_config = read_json(
        args.results_dir / stage3_phase_dir_name("development") / "run_config.json"
    )
    ...
    if git_mismatch and not args.allow_git_mismatch:
        raise RuntimeError(
            "The repository state changed after the seed-46 development run "
    ...
    write_stage3_final_decision(analysis, args.results_dir)
    print(f"Final decision: {analysis['final_decision']['decision']}")
    write_stage3_final_summary(args.results_dir, analysis)
    ...
    print(
        "Run scripts/audit_measured_damage_preservation.py to independently "
