#!/usr/bin/env python3
"""Build the new Stage 3 seed-46 development split (data only, no model NLL).
...
split, the contaminated seed-45 Stage 2C development split, and the untouched
seed-44 final split. Seed-44 artifacts are verified by hash only; no model
...
from expert_analysis.measured_damage_evaluation import build_stage3_development_split
...
def main() -> int:
    ...
    preflight = verify_stage3_upstream(args.results_root)
    print(
        "Frozen upstream decisions verified: STRONG GO / GO / SURROGATE_NO_GO / "
        "ROBUST_PRESERVATION_NO_GO / FRAGILITY_ROBUST_NO_GO"
    )
    seed44 = verify_seed44_untouched_stage3(args.results_root, args.output_dir)
    print("Seed-44 final split verified untouched (hashes only, no evaluation).")
    atomic_write_json(
        args.output_dir / "audits" / "stage3_preflight.json",
    ...
    manifest = build_stage3_development_split(
        source,
        tokenizer,
        args.stage2b_dir,
        args.stage2c_dir,
    ...
    for domain, entry in manifest["domains"].items():
        print(
            f"[{domain}] seed-46 examples: {entry['num_examples']}, prior pool "
